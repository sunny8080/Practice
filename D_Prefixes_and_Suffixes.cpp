...
public:
    void solve(string& s) {
        ...
        for (int i = s.size() - 1; i >= 0; i--) {
            if (z[i] + i == s.size()) lenPS.push_back(z[i]);
        }
        lenPS.push_back(s.size());

        vi dp(n + 1, 0); // dp[l] => no of times prefix of length l came as a substring

        for (auto len : z) dp[len]++;

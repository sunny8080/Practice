...
public:
    int solve() {
        int ans = INT_MAX;
        map<int, int> mpp;
        for (int i = 0; i < n;i++) mpp[p[i]] = i;
        for (int i = 0; i < m; i++) a[i] = mpp[a[i]];

        for (int i = 0; i < m - 1; i++) {
            if (a[i] >= a[i + 1] || a[i + 1] > a[i] + d) return 0;
        }
        for (int i = 0; i < m - 1; i++) {
            if (a[i] + d + 1 <= n - 1) ans = min(ans, a[i] + d + 1 - a[i + 1]);
            if (a[i + 1] - d>= 0) {
                ans = min(ans, a[i] - a[i + 1] + d);
                // cout << "efasd";
            }
        }
        return ans;

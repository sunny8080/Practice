...
int32_t main() {
    ...
    int n = s.size(), hash = 0, revHash = 0, p = 31, pwr = 1, sum = 0;
    int dp[n]={0};

    dp[0] = 1, hash = revHash = s[0], sum+=1, pwr = (pwr * p) % mod;

    for (int i = 1; i < n; i++, pwr = (pwr * p) % mod) {
        hash = (hash + s[i] * pwr) % mod;
        revHash = (revHash * p + s[i]) % mod;
        dp[i] = hash == revHash ? dp[(i - 1) / 2] + 1 : 0;
        sum += dp[i];
    }

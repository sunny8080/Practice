...
public:
    int solve(vi& nums) {
        int n = nums.size();
        // dp1[i] :- max possible sum of a subsequence of first i elements (nums[0...i-1]) where length of subsequence is always odd  
        // dp2[i] :- max possible sum of a subsequence of first i elements (nums[0...i-1]) where length of subsequence is always even  
        vi dp1(n + 1), dp2(n + 1);
        dp1[0] = (int)-1e18;
        dp2[0] = 0;

...
int32_t main() {
    ...
    map<string, int> mpp = { {"A", 1e6}, {"B", 1e6}, {"C", 1e6}, {"AB", 1e6}, {"AC", 1e6}, {"BC", 1e6}, {"ABC", 1e6} };
    ...
    // cout << mpp["A"] << sp << mpp["B"] << sp << mpp["C"] << nl;

    mpp["ABC"] = min({ mpp["ABC"], mpp["AB"] + mpp["C"], mpp["BC"] + mpp["A"], mpp["AC"] + mpp["B"], mpp["A"] + mpp["B"] + mpp["C"] });
    cout << (mpp["ABC"] == 1e6 ? -1 : mpp["ABC"]);

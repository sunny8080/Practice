...
class Solution {
    int help(int ind, vi& nums, vi& dp) {
        if (ind == nums.size()) return 1;
        if (dp[ind] != -1) return dp[ind];

        int cnt = 0;
        for (int i = 2 * ind; i < nums.size(); i += ind) {
            if (nums[i] > nums[ind]) cnt = max(cnt, help(i, nums, dp));
        }
        return dp[ind] = max(dp[ind], cnt + 1);
    }
